...
public class MaxCoverFixedOffline extends MaxCover {
    ...
    public MaxCoverFixedOffline(ArrayList container,
            Integer currentTI) {
    ...
    public HashSet<Integer> maxCover() {
        ...
        budgetPerInstance[budgetPerInstance.length - 1] = budget - budget / numberTimeInstance * (numberTimeInstance - 1);
        ...
        while (assignWorkers.size() < budget && !Q.isEmpty()) {
            ...
            for (int k : S.keySet()) {
                ...
                if (selectedWorkerAtTimeInstance.get(w.getOnlineTime()) != null
                        && selectedWorkerAtTimeInstance.get(w.getOnlineTime()) >= budgetPerInstance[w.getOnlineTime()]) {
                    continue;
                }
...
//			System.out.println(maxNoUncoveredTasks);
            ...
            SensingWorker w = (SensingWorker) workerList.get(bestWorkerIndex);
            if (selectedWorkerAtTimeInstance.get(w.getOnlineTime()) != null) {
                selectedWorkerAtTimeInstance.put(w.getOnlineTime(),
                        selectedWorkerAtTimeInstance.get(w.getOnlineTime()) + 1);
            } else {
...
//        System.out.println(universe.size() + "\t" + assignedTasks + "\t" + assignWorkers.size() + "\t" + assignedTasks / assignWorkers.size());
        for(Integer i:selectedWorkerAtTimeInstance.keySet()){
            System.out.println("#Selected workers in Time instance "+(i+1)+":"+selectedWorkerAtTimeInstance.get(i));
        }
